...
from mmdet.core import images_to_levels, multi_apply, multiclass_nms, unmap
...
class DCNConvModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = ModulatedDeformConv2dPack(in_channels, out_channels,
                                              kernel_size, 1, dcn_pad,
                                              dilation)
...
class PyCenterNetHead(AnchorFreeHead):
    ...
    def __init__(
            ...
            loss_cls=dict(use_sigmoid=True,
                          ...
                          alpha=0.25,
                          loss_weight=1.0),
            loss_bbox_init=dict(beta=1.0 / 9.0, loss_weight=0.5),
            loss_bbox_refine=dict(beta=1.0 / 9.0, loss_weight=1.0),
            loss_heatmap=dict(alpha=2.0, gamma=4.0, loss_weight=0.25),
            loss_offset=dict(beta=1.0 / 9.0, loss_weight=1.0),
            loss_sem=dict(use_sigmoid=True,
                          gamma=2.0,
                          alpha=0.25,
                          loss_weight=0.1),
            **kwargs):
        ...
        if self.train_cfg:
            ...
            self.hm_assigner = build_assigner(
                self.train_cfg["heatmap"]["assigner"])
        ...
        self.loss_tl_cls = FocalLoss(**loss_cls)
        self.loss_br_cls = FocalLoss(**loss_cls)
        self.loss_tl_bbox_init = SmoothL1Loss(**loss_bbox_init)
        self.loss_tl_bbox_refine = SmoothL1Loss(**loss_bbox_refine)
        self.loss_br_bbox_init = SmoothL1Loss(**loss_bbox_init)
        self.loss_br_bbox_refine = SmoothL1Loss(**loss_bbox_refine)
        self.loss_heatmap = GaussianFocalLoss(**loss_heatmap)
        self.loss_offset = SmoothL1Loss(**loss_offset)
        self.loss_sem = SEPFocalLoss(**loss_sem)
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            if self.conv_module_style == 'norm':
                ...
                self.tl_cls_convs.append(
                    DCNConvModule(chn, self.feat_channels, self.dcn_kernel, 1,
        ...
        self.hem_tl = TLPool(self.feat_channels,
                             self.conv_cfg,
                             self.norm_cfg,
                             first_kernel_size=self.first_kernel_size,
                             kernel_size=self.kernel_size,
                             corner_dim=self.corner_dim)
        self.hem_br = BRPool(self.feat_channels,
                             self.conv_cfg,
                             self.norm_cfg,
                             first_kernel_size=self.first_kernel_size,
                             kernel_size=self.kernel_size,
                             corner_dim=self.corner_dim)
        self.hem_ct = ConvModule(self.feat_channels,
                                 self.feat_channels,
                                 3,
                                 stride=1,
                                 padding=1,
        ...
        self.pycenter_tl_cls_conv = DeformConv2d(cls_in_channels,
                                                 self.point_feat_channels,
                                                 self.dcn_kernel, 1,
                                                 self.dcn_pad)
        self.pycenter_tl_cls_out = nn.Conv2d(self.point_feat_channels,
        ...
        self.pycenter_br_cls_conv = DeformConv2d(cls_in_channels,
                                                 ...
                                                 self.dcn_kernel, 1,
                                                 self.dcn_pad)
        self.pycenter_br_cls_out = nn.Conv2d(self.point_feat_channels,
                                             self.cls_out_channels, 1, 1, 0)

        self.pycenter_tl_pts_init_conv = nn.Conv2d(self.feat_channels,
                                                   self.point_feat_channels, 3,
                                                   1, 1)
        self.pycenter_tl_pts_init_out = nn.Conv2d(self.point_feat_channels,
                                                  pts_out_dim, 1, 1, 0)

        self.pycenter_br_pts_init_conv = nn.Conv2d(self.feat_channels,
                                                   self.point_feat_channels, 3,
                                                   1, 1)
        self.pycenter_br_pts_init_out = nn.Conv2d(self.point_feat_channels,
                                                  pts_out_dim, 1, 1, 0)
        pts_in_channels = self.feat_channels + 9
        self.pycenter_tl_pts_refine_conv = DeformConv2d(
            pts_in_channels, self.point_feat_channels, self.dcn_kernel, 1,
            self.dcn_pad)
        self.pycenter_tl_pts_refine_out = nn.Conv2d(self.point_feat_channels,
                                                    pts_out_dim, 1, 1, 0)

        self.pycenter_br_pts_refine_conv = DeformConv2d(
            pts_in_channels, self.point_feat_channels, self.dcn_kernel, 1,
            self.dcn_pad)
        self.pycenter_br_pts_refine_out = nn.Conv2d(self.point_feat_channels,
                                                    pts_out_dim, 1, 1, 0)

        self.pycenter_hem_tl_score_out = nn.Conv2d(self.feat_channels, 1, 3, 1,
                                                   1)
        self.pycenter_hem_br_score_out = nn.Conv2d(self.feat_channels, 1, 3, 1,
                                                   1)
        self.pycenter_hem_ct_score_out = nn.Conv2d(self.feat_channels, 1, 3, 1,
                                                   1)
        self.pycenter_hem_tl_offset_out = nn.Conv2d(self.feat_channels, 2, 3,
                                                    1, 1)
        self.pycenter_hem_br_offset_out = nn.Conv2d(self.feat_channels, 2, 3,
                                                    1, 1)
        self.pycenter_hem_ct_offset_out = nn.Conv2d(self.feat_channels, 2, 3,
                                                    1, 1)

        self.pycenter_sem_out = nn.Conv2d(self.feat_channels,
                                          self.cls_out_channels, 1, 1, 0)
        self.pycenter_sem_embedding = ConvModule(self.feat_channels,
                                                 self.feat_channels,
                                                 1,
                                                 conv_cfg=self.conv_cfg,
                                                 norm_cfg=self.norm_cfg)

    def update(self, train_cfg=None, test_cfg=None):
        if train_cfg is not None:
            self.train_cfg = train_cfg
            self.init_assigner = build_assigner(train_cfg["init"]["assigner"])
            self.refine_assigner = build_assigner(
                train_cfg["refine"]["assigner"])
            self.hm_assigner = build_assigner(train_cfg["heatmap"]["assigner"])
            # use PseudoSampler when sampling is False
            sampler_cfg = dict(type='PseudoSampler')
            self.sampler = build_sampler(sampler_cfg, context=self)
    ...
    def forward_single(self, x):
        ...
        hem_score_out = torch.cat(
            [hem_tl_score_out, hem_br_score_out, hem_ct_score_out], dim=1)
        hem_offset_out = torch.cat(
            [hem_tl_offset_out, hem_br_offset_out, hem_ct_offset_out], dim=1)
        ...
        tl_cls_out = self.pycenter_tl_cls_out(
            self.relu(self.pycenter_tl_cls_conv(tl_cls_feat, dcn_tl_offset)))
        br_cls_out = self.pycenter_br_cls_out(
            self.relu(self.pycenter_br_cls_conv(br_cls_feat, dcn_br_offset)))
        pts_tl_out_refine = self.pycenter_tl_pts_refine_out(
            self.relu(
                self.pycenter_tl_pts_refine_conv(tl_pts_feat, dcn_tl_offset)))
        pts_br_out_refine = self.pycenter_br_pts_refine_out(
            self.relu(
                self.pycenter_br_pts_refine_conv(br_pts_feat, dcn_br_offset)))

        return (tl_cls_out, br_cls_out, pts_tl_out_init, pts_br_out_init,
                pts_tl_out_refine, pts_br_out_refine, hem_score_out,
                hem_offset_out, sem_scores_out)
    ...
    def _point_target_single(self,
        ...
        if unmap_outputs:
            ...
            bbox_weights = unmap(bbox_weights, num_total_proposals,
                                 inside_flags)
    ...
    def _hm_target_single(self,
        ...
        offset_tl_weights = points.new_zeros([num_valid_points, 2],
                                             dtype=torch.float)
        offset_br_weights = points.new_zeros([num_valid_points, 2],
                                             dtype=torch.float)
        offset_ct_weights = points.new_zeros([num_valid_points, 2],
                                             dtype=torch.float)
        ...
        if unmap_outputs:
            ...
            hm_tl_weights = unmap(hm_tl_weights, num_total_points,
                                  inside_flags)
            offset_tl_weights = unmap(offset_tl_weights, num_total_points,
                                      inside_flags)

            gt_hm_br = unmap(gt_hm_br, num_total_points, inside_flags)
            gt_offset_br = unmap(gt_offset_br, num_total_points, inside_flags)
            hm_br_weights = unmap(hm_br_weights, num_total_points,
                                  inside_flags)
            offset_br_weights = unmap(offset_br_weights, num_total_points,
                                      inside_flags)

            gt_hm_ct = unmap(gt_hm_ct, num_total_points, inside_flags)
            gt_offset_ct = unmap(gt_offset_ct, num_total_points, inside_flags)
            hm_ct_weights = unmap(hm_ct_weights, num_total_points,
                                  inside_flags)
            offset_ct_weights = unmap(offset_ct_weights, num_total_points,
                                      inside_flags)

        return (gt_hm_tl, gt_offset_tl, hm_tl_weights, offset_tl_weights,
                pos_inds_tl, neg_inds_tl, gt_hm_br, gt_offset_br,
                hm_br_weights, offset_br_weights, pos_inds_br, neg_inds_br,
                gt_hm_ct, gt_offset_ct, hm_ct_weights, offset_ct_weights,
                pos_inds_ct, neg_inds_ct)
    ...
    def get_hm_targets(self,
        ...
        if any([gt_hm_tl is None for gt_hm_tl in all_gt_hm_tl]):
            return None
        # sampled points of all images
        num_total_pos_tl = sum(
            [max(inds.numel(), 1) for inds in pos_inds_tl_list])
        num_total_neg_tl = sum(
            [max(inds.numel(), 1) for inds in neg_inds_tl_list])
        num_total_pos_br = sum(
            [max(inds.numel(), 1) for inds in pos_inds_br_list])
        num_total_neg_br = sum(
            [max(inds.numel(), 1) for inds in neg_inds_br_list])
        num_total_pos_ct = sum(
            [max(inds.numel(), 1) for inds in pos_inds_ct_list])
        num_total_neg_ct = sum(
            [max(inds.numel(), 1) for inds in neg_inds_ct_list])

        gt_hm_tl_list = images_to_levels(all_gt_hm_tl, num_level_proposals)
        gt_offset_tl_list = images_to_levels(all_gt_offset_tl,
                                             num_level_proposals)
        hm_tl_weight_list = images_to_levels(all_hm_tl_weights,
                                             num_level_proposals)
        offset_tl_weight_list = images_to_levels(all_offset_tl_weights,
                                                 num_level_proposals)

        gt_hm_br_list = images_to_levels(all_gt_hm_br, num_level_proposals)
        gt_offset_br_list = images_to_levels(all_gt_offset_br,
                                             num_level_proposals)
        hm_br_weight_list = images_to_levels(all_hm_br_weights,
                                             num_level_proposals)
        offset_br_weight_list = images_to_levels(all_offset_br_weights,
                                                 num_level_proposals)

        gt_hm_ct_list = images_to_levels(all_gt_hm_ct, num_level_proposals)
        gt_offset_ct_list = images_to_levels(all_gt_offset_ct,
                                             num_level_proposals)
        hm_ct_weight_list = images_to_levels(all_hm_ct_weights,
                                             num_level_proposals)
        offset_ct_weight_list = images_to_levels(all_offset_ct_weights,
                                                 num_level_proposals)

        return (gt_hm_tl_list, gt_offset_tl_list, hm_tl_weight_list,
                offset_tl_weight_list, gt_hm_br_list, gt_offset_br_list,
                hm_br_weight_list, offset_br_weight_list, gt_hm_ct_list,
                gt_offset_ct_list, hm_ct_weight_list, offset_ct_weight_list,
                num_total_pos_tl, num_total_neg_tl, num_total_pos_br,
                num_total_neg_br, num_total_pos_ct, num_total_neg_ct)

    def loss_single(
            self, tl_cls_score, br_cls_score, tl_pts_pred_init,
            br_pts_pred_init, tl_pts_pred_refine, br_pts_pred_refine, hm_score,
            hm_offset, tl_labels, br_labels, tl_label_weights,
            br_label_weights, tl_bbox_gt_init, br_bbox_gt_init,
            tl_bbox_weights_init, br_bbox_weights_init, tl_bbox_gt_refine,
            br_bbox_gt_refine, tl_bbox_weights_refine, br_bbox_weights_refine,
            ...
            stride, tl_num_total_samples_init, br_num_total_samples_init,
        ...
        tl_cls_score = tl_cls_score.permute(0, 2, 3,
                                            1).reshape(-1,
                                                       self.cls_out_channels)
        br_cls_score = br_cls_score.permute(0, 2, 3,
                                            1).reshape(-1,
                                                       self.cls_out_channels)

        loss_cls = 0

        loss_cls += self.loss_tl_cls(tl_cls_score,
                                     tl_labels,
                                     tl_label_weights,
                                     avg_factor=tl_num_total_samples_refine)

        loss_cls += self.loss_br_cls(br_cls_score,
                                     br_labels,
                                     br_label_weights,
        ...
        tl_bbox_pred_init = self.points2bbox(tl_pts_pred_init.reshape(
            -1, 2 * self.num_points),
                                             y_first=False)
        tl_bbox_gt_refine = tl_bbox_gt_refine.reshape(-1, 4)
        tl_bbox_weights_refine = tl_bbox_weights_refine.reshape(-1, 4)
        tl_bbox_pred_refine = self.points2bbox(tl_pts_pred_refine.reshape(
            -1, 2 * self.num_points),
                                               y_first=False)
        ...
        br_bbox_pred_init = self.points2bbox(br_pts_pred_init.reshape(
            -1, 2 * self.num_points),
                                             y_first=False)
        br_bbox_gt_refine = br_bbox_gt_refine.reshape(-1, 4)
        br_bbox_weights_refine = br_bbox_weights_refine.reshape(-1, 4)
        br_bbox_pred_refine = self.points2bbox(br_pts_pred_refine.reshape(
            -1, 2 * self.num_points),
                                               y_first=False)
        normalize_term = self.point_base_scale * stride
        ...
        hm_score_tl, hm_score_br, hm_score_ct = torch.chunk(hm_score,
                                                            3,
                                                            dim=-1)
        ...
        loss_heatmap += self.loss_heatmap(hm_score_ct,
                                          gt_hm_ct,
                                          gt_hm_ct_weight,
        ...
        hm_offset_tl, hm_offset_br, hm_offset_ct = torch.chunk(hm_offset,
                                                               3,
                                                               dim=-1)
        ...
        loss_offset = 0
        loss_offset += self.loss_offset(hm_offset_tl,
                                        gt_offset_tl,
                                        gt_offset_tl_weight,
                                        avg_factor=num_total_samples_tl)
        loss_offset += self.loss_offset(hm_offset_br,
                                        gt_offset_br,
                                        gt_offset_br_weight,
                                        avg_factor=num_total_samples_br)
        loss_offset += self.loss_offset(hm_offset_ct,
                                        gt_offset_ct,
                                        gt_offset_ct_weight,
                                        avg_factor=num_total_samples_ct)
        loss_offset /= 3.0
    ...
    def loss(self,
        ...
        candidate_list, valid_flag_list = self.get_points(
            featmap_sizes, img_metas)
        pts_tl_coordinate_preds_init = self.offset_to_pts(
            candidate_list, tl_pts_preds_init)
        pts_br_coordinate_preds_init = self.offset_to_pts(
            candidate_list, br_pts_preds_init)
        ...
        (*_, tl_bbox_gt_list_init, tl_bbox_weights_list_init,
         tl_num_total_pos_init,
         tl_num_total_neg_init) = tl_cls_reg_targets_init
        ...
        (*_, br_bbox_gt_list_init, br_bbox_weights_list_init,
         br_num_total_pos_init,
         br_num_total_neg_init) = br_cls_reg_targets_init

        # target for heatmap in initial stage
        proposal_list, valid_flag_list = self.get_points(
            featmap_sizes, img_metas)
        heatmap_targets = self.get_hm_targets(proposal_list, valid_flag_list,
                                              gt_bboxes, img_metas, gt_labels)
        (gt_hm_tl_list, gt_offset_tl_list, gt_hm_tl_weight_list,
         gt_offset_tl_weight_list, gt_hm_br_list, gt_offset_br_list,
         gt_hm_br_weight_list, gt_offset_br_weight_list, gt_hm_ct_list,
         gt_offset_ct_list, gt_hm_ct_weight_list, gt_offset_ct_weight_list,
         num_total_pos_tl, num_total_neg_tl, num_total_pos_br,
         num_total_neg_br, num_total_pos_ct,
         num_total_neg_ct) = heatmap_targets

        # target for refinement stage
        center_list, valid_flag_list = self.get_points(featmap_sizes,
                                                       img_metas)
        pts_tl_coordinate_preds_refine = self.offset_to_pts(
            center_list, tl_pts_preds_refine)
        pts_br_coordinate_preds_refine = self.offset_to_pts(
            center_list, br_pts_preds_refine)
        tl_bbox_list = []
        br_bbox_list = []
        ...
        tl_cls_reg_targets_refine = self.get_targets(
            ...
            stage='refine',
            label_channels=label_channels)
        (tl_labels_list, tl_label_weights_list, tl_bbox_gt_list_refine,
         tl_bbox_weights_list_refine, tl_num_total_pos_refine,
         tl_num_total_neg_refine) = tl_cls_reg_targets_refine
        ...
        br_cls_reg_targets_refine = self.get_targets(
            ...
            stage='refine',
            label_channels=label_channels)
        (br_labels_list, br_label_weights_list, br_bbox_gt_list_refine,
         br_bbox_weights_list_refine, br_num_total_pos_refine,
         br_num_total_neg_refine) = br_cls_reg_targets_refine
        ...
        for i in range(5):
            ...
            gt_lvl_sem_map = F.interpolate(gt_sem_map,
                                           sem_score.shape[-2:]).reshape(-1)
            gt_lvl_sem_weight = F.interpolate(gt_sem_weights,
                                              sem_score.shape[-2:]).reshape(-1)
            sem_score = sem_score.reshape(-1)

            try:
                concat_sem_scores = torch.cat([concat_sem_scores, sem_score])
                concat_gt_sem_map = torch.cat(
                    [concat_gt_sem_map, gt_lvl_sem_map])
                concat_gt_sem_weights = torch.cat(
        ...
        loss_sem = self.loss_sem(concat_sem_scores,
                                 concat_gt_sem_map,
                                 concat_gt_sem_weights,
    ...
    def get_bboxes(self,
        ...
        tl_bbox_preds_refine = [
            self.points2bbox(tl_pts_pred_refine)
            for tl_pts_pred_refine in tl_pts_preds_refine
        ]
        br_bbox_preds_refine = [
            self.points2bbox(br_pts_pred_refine)
            for br_pts_pred_refine in br_pts_preds_refine
        ]
        ...
        for img_id in range(len(img_metas)):
            ...
            tl_bbox_pred_list = [
                tl_bbox_preds_refine[i][img_id].detach()
                for i in range(num_levels)
            ]
            br_bbox_pred_list = [
                br_bbox_preds_refine[i][img_id].detach()
                for i in range(num_levels)
            ]
            hm_scores_list = [
            ...
            proposals = self._get_bboxes_single(
                tl_cls_score_list, br_cls_score_list, tl_bbox_pred_list,
                br_bbox_pred_list, hm_scores_list, hm_offsets_list,
    ...
    def _get_bboxes_single(self,
        ...
        def select(score_map, x, y, ks=2, i=0):
            ...
            x_round = round_func(
                (x / self.point_strides[i]).clamp(min=0,
                                                  max=score_map.shape[-1] - 1))
            y_round = round_func(
                (y / self.point_strides[i]).clamp(min=0,
                                                  max=score_map.shape[-2] - 1))

            select_indices = indices[y_round.to(torch.long),
                                     x_round.to(torch.long)]
        ...
        for i_lvl, (tl_cls_score, br_cls_score, tl_bbox_pred, br_bbox_pred,
                    points) in enumerate(
                        zip(tl_cls_scores, br_cls_scores, tl_bbox_preds,
                            br_bbox_preds, mlvl_points)):
            assert tl_cls_score.size()[-2:] == tl_bbox_pred.size()[-2:]
            tl_scores = tl_cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).sigmoid()
            br_scores = br_cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).sigmoid()
            ...
            tl_bbox_pos_center = torch.cat(
                [tl_points[:, :2], tl_points[:, :2]], dim=1)
            br_bbox_pos_center = torch.cat(
                [br_points[:, :2], br_points[:, :2]], dim=1)
            tl_bboxes = tl_bbox_pred * self.point_strides[
                i_lvl] + tl_bbox_pos_center
            br_bboxes = br_bbox_pred * self.point_strides[
                i_lvl] + br_bbox_pos_center
            ...
            if i_lvl > 0:
                ...
                x1_new, y1_new, score1_new = select(hm_scores[i][0, ...], x1,
                                                    y1, 2, i)
                x2_new, y2_new, score2_new = select(hm_scores[i][2, ...], x2,
                                                    y2, 2, i)
                x3_new, y3_new, score3_new = select(hm_scores[i][2, ...], x3,
                                                    y3, 2, i)
                x4_new, y4_new, score4_new = select(hm_scores[i][1, ...], x4,
                                                    y4, 2, i)

                hm_offset = hm_offsets[i].permute(1, 2, 0)
                point_stride = self.point_strides[i]

                x1 = ((x1_new + hm_offset[y1_new.to(torch.long),
                                          x1_new.to(torch.long), 0]) *
                      point_stride).clamp(min=0, max=img_shape[1])
                y1 = ((y1_new + hm_offset[y1_new.to(torch.long),
                                          x1_new.to(torch.long), 1]) *
                      point_stride).clamp(min=0, max=img_shape[0])
                x2 = ((x2_new + hm_offset[y2_new.to(torch.long),
                                          x2_new.to(torch.long), 4]) *
                      point_stride).clamp(min=0, max=img_shape[1])
                y2 = ((y2_new + hm_offset[y2_new.to(torch.long),
                                          x2_new.to(torch.long), 5]) *
                      point_stride).clamp(min=0, max=img_shape[0])
                x3 = ((x3_new + hm_offset[y3_new.to(torch.long),
                                          x3_new.to(torch.long), 4]) *
                      point_stride).clamp(min=0, max=img_shape[1])
                y3 = ((y3_new + hm_offset[y3_new.to(torch.long),
                                          x3_new.to(torch.long), 5]) *
                      point_stride).clamp(min=0, max=img_shape[0])
                x4 = ((x4_new + hm_offset[y4_new.to(torch.long),
                                          x4_new.to(torch.long), 2]) *
                      point_stride).clamp(min=0, max=img_shape[1])
                y4 = ((y4_new + hm_offset[y4_new.to(torch.long),
                                          x4_new.to(torch.long), 3]) *
                      point_stride).clamp(min=0, max=img_shape[0])
        ...
        det_tl_bboxes, det_tl_labels = multiclass_nms(mlvl_tl_bboxes,
                                                      mlvl_tl_scores,
                                                      cfg["score_thr"],
                                                      cfg["nms"],
                                                      cfg["max_per_img"])
        det_br_bboxes, det_br_labels = multiclass_nms(mlvl_br_bboxes,
                                                      mlvl_br_scores,
                                                      cfg["score_thr"],
                                                      cfg["nms"],
                                                      cfg["max_per_img"])

        det_bboxes, det_scores, det_labels = self.decode(
            det_tl_bboxes,
            det_tl_labels,
            det_br_bboxes,
            det_br_labels,
    ...
    def get_num_level_proposals_inside(self, num_level_proposals,
                                       inside_flags):
    ...
    def decode(self,
        ...
        tl_scores = tl_scores.view(tl_scores.size(0),
                                   1).repeat(1, br_scores.size(0))
        br_scores = br_scores.view(1, br_scores.size(0)).repeat(
            tl_scores.size(0), 1)
        scores = (tl_scores + br_scores) / 2

        tl_clses = det_tl_labels.view(det_tl_labels.size(0),
                                      1).repeat(1, det_br_labels.size(0))
        br_clses = det_br_labels.view(1, det_br_labels.size(0)).repeat(
            det_tl_labels.size(0), 1)
        ...
        rcentral[
            ...,
            0] = bboxes_center_x - mu * (bboxes[..., 2] - bboxes[..., 0]) / 2
        rcentral[
            ...,
            1] = bboxes_center_y - mu * (bboxes[..., 3] - bboxes[..., 1]) / 2
        rcentral[
            ...,
            2] = bboxes_center_x + mu * (bboxes[..., 2] - bboxes[..., 0]) / 2
        rcentral[
            ...,
            3] = bboxes_center_y + mu * (bboxes[..., 3] - bboxes[..., 1]) / 2
