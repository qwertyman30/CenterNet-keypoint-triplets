...
class PyCenterNetHead(AnchorFreeHead):
    def __init__(self,
                 ...
                 loss_sem=dict(use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.1),
                 **kwargs):
    ...
    def _init_layers(self):
        ...
        self.hem_br = BRPool(self.feat_channels, self.conv_cfg, self.norm_cfg,
                             first_kernel_size=self.first_kernel_size, kernel_size=self.kernel_size,
                             corner_dim=self.corner_dim)
        self.hem_ct = ConvModule(self.feat_channels, self.feat_channels, 3, stride=1, padding=1,
                                 conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg)
    ...
    def update(self, train_cfg=None, test_cfg=None):
        if train_cfg is not None:
            ...
            self.refine_assigner = build_assigner(train_cfg["refine"]["assigner"])
