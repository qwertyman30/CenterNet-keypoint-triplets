...
def get_official_eval_result(gt_annos, dt_annos, current_classes):
    ...
    for j, curcls in enumerate(current_classes):
        ...
        cls_key = f"{curcls}: {class_to_name[curcls]}"
        ret_dict[cls_key] = OrderedDict()
        for i in range(min_overlaps.shape[0]):
            ret_dict[cls_key][f"bbox_{min_overlaps[i, 0, j]:.2f}"] = (
                mAPbbox[j, 0, i], mAPbbox[j, 1, i], mAPbbox[j, 2, i])
            ret_dict[cls_key][f"bev_{min_overlaps[i, 1, j]:.2f}"] = (
                mAPbev[j, 0, i], mAPbev[j, 1, i], mAPbev[j, 2, i])
            ret_dict[cls_key][f"3d_{min_overlaps[i, 2, j]:.2f}"] = (
                mAP3d[j, 0, i], mAP3d[j, 1, i], mAP3d[j, 2, i])
            if compute_aos:
                ret_dict[cls_key][f"aos"] = (
                    mAPaos[j, 0, i], mAPaos[j, 1, i], mAPaos[j, 2, i])
    return ret_dict
